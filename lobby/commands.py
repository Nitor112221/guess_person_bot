...
async def create_lobby(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if result['success']:
        ...
        keyboard = [
            [
                InlineKeyboardButton("📋 Копировать код", 
...
async def process_invite_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if result['success']:
        ...
        keyboard = [
            [InlineKeyboardButton("📊 Информация о лобби", 
...
async def my_lobby_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if lobby_info['host_id'] == user_id:
        keyboard.append([
            InlineKeyboardButton("🎮 Начать игру", 
                               callback_data=f"start_{lobby_info['lobby_id']}")
        ])
    ...
    keyboard.append([
        ...
        InlineKeyboardButton("🚪 Выйти", 
                           callback_data=f"leave_{lobby_info['lobby_id']}")
    ])
...
async def leave_lobby(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [
            InlineKeyboardButton("✅ Да, выйти", 
                               callback_data=f"confirm_leave_{lobby_id}"),
            InlineKeyboardButton("❌ Нет, остаться", 
